...
func publishModuleVersion(t *testing.T, client *http.Client, baseURL, namespaceName, moduleName, providerName, version string) int {
	...
	uploadURL := fmt.Sprintf("%s/v1/terrareg/modules/%s/%s/%s/upload", baseURL, namespaceName, moduleName, providerName, version)
...
// TestAPICompatibility compares critical API responses between Go and Python implementations
...
// TestWebhookIntegration tests webhook functionality for all Git providers
